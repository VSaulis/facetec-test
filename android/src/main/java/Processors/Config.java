...
public class Config {
  ...
  public static String DeviceKeyIdentifier = "dMV5Lvfy6LFjIQKBREZ5X3Od2RjjxMzd";
  ...
  public static void initializeFaceTecSDK(Context context, String productionKeyText, String deviceKeyIdentifier, String faceScanEncryptionKey, FaceTecSDK.InitializeCallback callback) {
    if (deviceKeyIdentifier != null && !deviceKeyIdentifier.trim().isEmpty() &&
      faceScanEncryptionKey != null && !faceScanEncryptionKey.trim().isEmpty() &&
      productionKeyText != null && !productionKeyText.trim().isEmpty()) {
      Log.d("ReactNativeFaceTec", "Initializing FaceTecSDK in production mode with these values:" +
        "\nproductionKeyText -> " + productionKeyText +
        ";\ndeviceKeyIdentifier -> " + deviceKeyIdentifier +
        ";\nfaceScanEncryptionKey -> " + faceScanEncryptionKey + ";");
      FaceTecSDK.initializeInProductionMode(context, productionKeyText, deviceKeyIdentifier, faceScanEncryptionKey, callback);
    } else {
      Log.d("ReactNativeFaceTec", "Initializing FaceTecSDK in development mode with default values: " +
        "\ndeviceKeyIdentifier -> " + DeviceKeyIdentifier +
