...
import com.reactnativefacetec.FacetecStatus;
import com.reactnativefacetec.FacetecViewModel;
...
public class PhotoIDMatchProcessor extends Processor implements FaceTecFaceScanProcessor, FaceTecIDScanProcessor {
  private final FacetecState facetecState = new FacetecState();
  FacetecViewModel viewModel;

  public PhotoIDMatchProcessor(String sessionToken, Context context) {
    viewModel = new ViewModelProvider((ViewModelStoreOwner) context).get(FacetecViewModel.class);
  ...
  public void processSessionWhileFaceTecSDKWaits(final FaceTecSessionResult sessionResult, final FaceTecFaceScanResultCallback faceScanResultCallback) {
    ...
    //
    viewModel.setLatestSessionResult(sessionResult);

    //
    // Part 3:  Handles early exit scenarios where there is no FaceScan to handle -- i.e. User Cancellation, Timeouts, etc.
    //
    if (sessionResult.getStatus() != FaceTecSessionStatus.SESSION_COMPLETED_SUCCESSFULLY) {
      facetecState.setStatus(FacetecStatus.CANCELLED);
    ...
    try {
      ...
      parameters.put("externalDatabaseRefID", viewModel.getLatestExternalDatabaseRefID());
    } catch (JSONException e) {
      e.printStackTrace();
      Log.d("ReactNativeFaceTec", "Exception raised while attempting to create JSON payload for upload.");
    }
      ...
      public void onResponse(@NonNull Call call, @NonNull okhttp3.Response response) throws IOException {
        ...
        try {
          ...
          if (wasProcessed) {
            ...
            facetecState.setStatus(faceScanResultCallback.proceedToNextStep(scanResultBlob) ? FacetecStatus.SUCCEEDED : FacetecStatus.FAILED);
          } else {
            // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
            facetecState.setStatus(FacetecStatus.FAILED);
            faceScanResultCallback.cancel();
          }
        } catch (JSONException e) {
          ...
          facetecState.setStatus(FacetecStatus.FAILED);
          e.printStackTrace();
          Log.d("ReactNativeFaceTec", "Exception raised while attempting to parse JSON result.");
          faceScanResultCallback.cancel();
        }
      }
  ...
  public void processIDScanWhileFaceTecSDKWaits(final FaceTecIDScanResult idScanResult, final FaceTecIDScanResultCallback idScanResultCallback) {
    ...
    // In the code in your own App, you can pass around signals, flags, intermediates, and results however you would like.
    //
    viewModel.setLatestIDScanResult(idScanResult);
      ...
      public void onResponse(@NonNull Call call, @NonNull okhttp3.Response response) throws IOException {
        ...
        try {
          ...
          if (wasProcessed) {
            ...
            facetecState.setStatus(idScanResultCallback.proceedToNextStep(scanResultBlob) ? FacetecStatus.SUCCEEDED : FacetecStatus.FAILED);
          } else {
            // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
            facetecState.setStatus(FacetecStatus.FAILED);
            idScanResultCallback.cancel();
  ...
  public FacetecState getLastState() {
