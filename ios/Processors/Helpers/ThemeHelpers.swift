...
class ThemeHelpers {
    ...
    public class func setAppTheme(theme: String) {
        RNOConfig.currentCustomization = getCustomizationForTheme(theme: theme)
        RNOConfig.currentLowLightCustomization = getLowLightCustomizationForTheme(theme: theme)
        RNOConfig.currentDynamicDimmingCustomization = getDynamicDimmingCustomizationForTheme(theme: theme)
        
        RNOUtilities.setVocalGuidanceSoundFiles()
        FaceTec.sdk.setCustomization(RNOConfig.currentCustomization)
        FaceTec.sdk.setLowLightCustomization(RNOConfig.currentLowLightCustomization)
        FaceTec.sdk.setDynamicDimmingCustomization(RNOConfig.currentDynamicDimmingCustomization)
    }
    
    class func getCustomizationForTheme(theme: String) -> FaceTecCustomization {
        var currentCustomization = FaceTecCustomization()
        
        let retryScreenSlideshowImages: [UIImage] = [] //[UIImage(named: "FaceTec_ideal_1")!, UIImage(named: "FaceTec_ideal_2")!, UIImage(named: "FaceTec_ideal_3")!, UIImage(named: "FaceTec_ideal_4")!, UIImage(named: "FaceTec_ideal_5")!]
        
        if theme == "FaceTec Theme" {
            // using default customizations -- do nothing
        }
        else if theme == "Config Wizard Theme" {
            currentCustomization = RNOConfig.retrieveConfigurationWizardCustomization()
        }
    ...
    class func getLowLightCustomizationForTheme(theme: String) -> FaceTecCustomization {
        ...
        if theme == "Config Wizard Theme" {
            currentLowLightCustomization = RNOConfig.retrieveLowLightConfigurationWizardCustomization()
        }
        else if theme == "Bitcoin Exchange" {
            let primaryColor = UIColor(red: 0.969, green: 0.588, blue: 0.204, alpha: 1) // orange
            let primaryColorLight = UIColor(red: 0.976, green: 0.702, blue: 0.424, alpha: 1) // light orange
    ...
    class func getDynamicDimmingCustomizationForTheme(theme: String) -> FaceTecCustomization {
        ...
        if theme == "FaceTec Theme" {
            ...
            currentDynamicDimmingCustomization = RNOConfig.retrieveDynamicDimmingConfigurationWizardCustomization()
        }
        else if theme == "Pseudo-Fullscreen" {
